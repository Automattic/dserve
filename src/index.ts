...
const logRejections = () => {
  ...
  Array.from(promiseRejections.entries())
    ...
    .forEach(([promise, [ts, reason]]) => {
      l.log(
        { reason },
        "Unhandled rejection sitting in queue for at least one minute"
      );
...
calypsoServer.get("*", async (req: any, res: any) => {
  ...
  const isCurrentlyBuilding =
    !hasLocally && (await isBuildInProgress(commitHash));
...
const server = calypsoServer.listen(3000, () =>
  l.log(
    `✅ dserve is listening on 3000 - started at ${startedServerAt.toLocaleTimeString(
      undefined,
